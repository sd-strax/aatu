...
| H5 | Every `request_action`'s `parameters` conform to the declared input schema (`08 §3`): no undeclared keys, required parameters present. Born from a real run (the model invented `{"title","body"}` for `ticket.create` instead of the declared `{summary, …}`). Grades **substance**, not encoding: it applies the loop's `parameters` normalization first (a model-stringified JSON object is unwrapped, `05 §3.4`), so a benign double-encoding the loop transparently handles does not fail, but wrong/missing keys do. The engine also rejects a malformed request at request time; the assertion grades that the agent asks for the right thing, not just that the wall held (the H3/A3 tripwire pattern, §7) | MUST | transcript |
