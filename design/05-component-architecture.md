...
- **Distributions are baked; the split root makes that safe.** Baking distributions into image
  layers under a volume-mounted root cannot work — the mount shadows them, and named-volume
  copy-up turns into **stale-binary shadowing on every image upgrade** (the volume's old
  distributions silently overriding the new image's). So the image relocates them entirely:
  `data.runtime_dir` (overridable by `$RECKON_RUNTIME_DIR`, which is how the image sets it
  without touching the volume-resident config) points each component's *immutable
  distribution* at an image-owned root, populated at build time by `reckon prefetch-runtimes`.
  Result: instant boot, no runtime egress, air-gap-clean, and an image upgrade cleanly replaces
  the binaries. Host installs leave `runtime_dir` unset and keep today's
  everything-under-the-data-root, download-on-first-run behavior; the same prefetch command
  prepares offline bundles for air-gapped host installs.
