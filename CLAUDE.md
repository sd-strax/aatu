...
The code is younger than the specs — most packages are skeletons with a `doc.go` stating which phase fills them in (`capability/` Phase B, `action/` Phase C, `knowledge/` Phase C/G, `identity/` Phase B). The wired-up spine today is the supervisor, the runtime/module seam, the event-sourced aggregate, the auth gates, the Temporal worker (`temporal/`, workflow bodies still Phase C+), and the observability layer (`telemetry/`).
