...
**Capability (`capability/`).** Phase B.2/B.4 so far: the transport-neutral `Adapter` interface (`03 §5.3`) with five classes and the `§6.2` error taxonomy (`RETRY`/`FALLTHROUGH`/`UNHEALTHY`/`FATAL` as `*AdapterError`, branched on by the future resolver), the telemetry-layer types (`OcsfPayload` → immutable `OcsfEvent` at ingest), and the `FixtureAdapter` replaying `fixtures/<scenario>/*.json` (`03 §9`). Invariants: pure I/O, no reasoning, never authors `x-interpretation`, `derivation_mode = DIRECT` (sole exception: the detection_finding normalizer, `03 §4.12`, still to land). Normalizer registry and the verb resolver (`03 §3`) are the remaining Phase B work.
