...
**Temporal worker (`temporal/`).** The in-process worker (`worker.go`) is a `supervisor.Component` that polls the `reckon` task queue — no separate worker binary (`05 §3.3`); it runs in the backend's process. Phase A.7 registers the full OSS workflow inventory (`ActionLifecycle`, `ReversalSaga`, `RenormalizePass`, `ArchiveInvestigation`, `PostConclusionPipeline`, `SummarizeForKnowledgeIndex`) under stable type names as skeletons that fail fast with a non-retryable `Unimplemented` ApplicationError, plus a trivial `Ping` workflow. The dispatch `Client` (`client.go`) is the command-path entry point; Phase C fills the workflow bodies and extends `Client` with real `Start*` methods. Note the package name clashes with the SDK's `go.temporal.io/sdk/temporal`, which is imported aliased as `sdktemporal`.
