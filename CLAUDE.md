...
**Knowledge service (`knowledge/`).** Phase C.5, `design/06`: the SOP corpus over the separate `reckon_knowledge` DB. `Store` does CRUD (`sop.go`, lightweight governance — published on write; identity is a tenant-scoped random UUIDv4, SOPs are documents not facts) and `RecallSOPs` (`retrieval.go`) — v0 keyword retrieval via Postgres full-text (`ts_rank` over title+body) with a tag hard-filter, returning ranked excerpts + `match_rationale` + `COMPLETE`/`EMPTY` coverage (EMPTY is evidence-of-absence). Embeddings/vector similarity and the concluded-investigation summary corpus are v1+/Phase G; the retrieval API shape (`06 §4`) is stable now. Exposed at `POST /api/knowledge/recall_sops` (reader) + `/api/sops` CRUD (analyst to write). Independent of the C.1–C.4 write path.
